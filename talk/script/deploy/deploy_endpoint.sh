...
do
	tput smso
	echo "-[Step 1.$CNT] Deploy $PRJ/endpoint to remote server $REMOTE_IP and restart $PRJ/endpoint."
	tput rmso
	select SEL_ITEM in "Yes" "No" "Exit"
	do
		case $SEL_ITEM in 
			"Yes")
				echo "Copying endpoint to $REMOTE_IP:/data/endpoint"
				scp /data/staging/$PRJ/build/endpoint root@$REMOTE_IP:/data/endpoint
				ssh root@$REMOTE_IP "export LANG=en_US.UTF-8;\
				cd /service;\
				svc -dk endpoint;\
				mv /opt/talk/bin/endpoint /opt/talk/bin/endpoint_$(date +%Y-%m-%d-%H_%M_%S).bak;\
				mv /data/endpoint /opt/talk/bin/endpoint;\
				svc -u endpoint;"
				echo "******* deploy endpoint to $REMOTE_IP OK! *********"
				break
				;;
			"No")
				break
