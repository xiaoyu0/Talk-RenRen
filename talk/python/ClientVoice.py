...
#ServerHost = '220.181.92.26'
ServerHost = '10.9.16.20'
ServerHost = 'localhost'
ServerHost = 'talk.apis.tk'
HostName = 'talk.m.renren.com'  
#HostName = 'talk.apis.tk'
ServerPort = 80 
ServerPort = 1234
TalkUri = '/talk'
SendUri = '/send'
...
class Client(threading.Thread):
	"""
	增加了__talkVoiceUpload(),sendVoice(),vid是我们自己指定的vid=thisisvid
	默认把当前目录下的voiceFile文件，发送三次，seq分别为1,2,3
	前两次mode=segment,最后一次mode=end
	另外使用了当前目录下的另一个模块Multipart
	"""
	def __init__(self) :
	...
	def __talkVoiceUpload(self, toid, seqid, mode):
		...
		print 'call Multipart.post_multipart...'
		response = Multipart.post_multipart('/api', param.items(), files)
		print 'after Multipart.post_multipart....'
	...
	def sendVoice(self, toid):
		...
		for seq in range(1, 4):
			...
			if seq != 3:
				...
				mode = 'end'
			self.__talkVoiceUpload(self.toid, seq, mode) 
