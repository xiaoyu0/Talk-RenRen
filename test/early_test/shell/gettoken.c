...
#include"../wy.h"  //after g++ compilation , it's ok!
...
static bool store_char(char * word , int maxword , int c , int * np);
 TOKEN gettoken(char *word , int maxword)
{
...
static bool  store_char(char * word, int maxword , int c , int * np)  //don't know why i can not compile successfully here
{
	word[(*np)++] = c;
	return true;
