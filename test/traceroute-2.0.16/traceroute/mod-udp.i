...
# 15 "mod-udp.c" 2
# 1 "/usr/include/netinet/udp.h" 1 3 4
# 68 "/usr/include/netinet/udp.h" 3 4
struct udphdr
{
  u_int16_t source;
  u_int16_t dest;
...
# 1 "traceroute.h" 1
# 11 "traceroute.h"
# 1 "/usr/include/clif.h" 1 3 4
# 13 "/usr/include/clif.h" 3 4
...
static int udp_default_init (const sockaddr_any *dest,
 ...
 curr_port = port_seq ? port_seq : 33434;
...
static void set_coverage (int sk) {
 int val = (sizeof (struct udphdr));

 if (setsockopt (sk, IPPROTO_UDPLITE, 10,
         &coverage, sizeof (coverage)) < 0
 ) error ("UDPLITE_SEND_CSCOV");

 if (setsockopt (sk, IPPROTO_UDPLITE, 11,
         &val, sizeof (val)) < 0
...
static CLIF_option udplite_options[] = {
 { 0, "coverage", "NUM", "Set udplite send coverage to %s (default is "
    ...
    CLIF_set_uint, &coverage, 0, (0x2000) },
 { 0, 0, 0, 0, 0, 0, 0, 0 }
};
...
static int udplite_init (const sockaddr_any *dest,
 ...
 if (!coverage) coverage = (sizeof (struct udphdr));
...
static void udp_send_probe (probe *pb, int ttl) {
 ...
 if (do_send (sk, data, *length_p, ((void *)0)) < 0) {
     close (sk);
     pb->send_time = 0;
     return;
 }
...
static void udp_recv_probe (int sk, int revents) {
 ...
 if (!(revents & (0x001 | 0x008)))
  return;

 recv_reply (sk, !!(revents & 0x008), udp_check_reply);
}
...
static void __init_default_ops (void) __attribute__ ((constructor));
static void __init_default_ops (void) { tr_register_module (&default_ops); };
...
static void __init_udp_ops (void) __attribute__ ((constructor)); static void __init_udp_ops (void) { tr_register_module (&udp_ops); };
...
static void __init_udplite_ops (void) __attribute__ ((constructor));
static void __init_udplite_ops (void) { tr_register_module (&udplite_ops); };
