...
struct probe_struct {
	...
	int final;
	sockaddr_any res;
	double send_time; // double is the return type of get_time() 
	double recv_time;
	int recv_ttl;
	int sk;
	int seq;
...
struct tr_module_struct { // 这个struct 代表一个什么？
	struct tr_module_struct *next;
	const char *name;
	int (*init) (const sockaddr_any *dest,
				unsigned int port_seq, size_t *packet_len);  //这里定义了4个函数指针
...
#define DEF_START_PORT	33434	/*  start for traditional udp method   */  // 在每个module的  init 函数中， 要用到这个
...
// 为什么把这个宏定义放到最后, 这里，声明用，  __attribute__ ((constructor)), 在 main() 之前调用,  ##是把两个连成一个字符串
#define TR_MODULE(MOD)	 static void __init_ ## MOD (void) __attribute__ ((constructor));	\
