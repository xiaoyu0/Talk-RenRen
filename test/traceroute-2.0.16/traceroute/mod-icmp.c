...
static void icmp_send_probe (probe *pb, int ttl) {
	...
	if (af == AF_INET) {  // 也就是说 icmp module 发的包的 data 是一个 struct icmp
...
static tr_module icmp_ops = {  // this initialization method  is available in C99 and as a GNU C extension, 可以先只初始化一部份,name,4个函数指针，user, every module defined
	.name = "icmp",   // every module has its name                           // its own  global tr_module varible
	.init = icmp_init,
