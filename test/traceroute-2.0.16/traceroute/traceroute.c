/* Copyright (c)  2006, 2007		Dmitry Butskoy <buc@citadel.stu.neva.ru> License:  GPL v2 or any later
...
#include <math.h>  //for poisson ,  log()
...
static unsigned int sim_probes = DEF_SIM_PROBES;  //在使用中用 -N  设置,一次性发出去去包的个数，在do_it()中可以看到实现
static unsigned int probes_per_hop = DEF_NUM_PROBES;

static char **gateways = NULL;  // init_ip_options 中使用到了这个全局变量
...
static double wait_secs = DEF_WAIT_SECS; //the time (in seconds) to wait for a response to a probe (default 5.0 sec)
static double send_secs = DEF_SEND_SECS;   //send_secs is the time interval of sending probes
static int mtudisc = 0;
static int backward = 0;

static sockaddr_any dst_addr = {{ 0, }, };       // 这个文件定义的自己的(static) dest 和  source 地址
static char *dst_name = NULL;   // 应该是目标  域名 
static char *device = NULL;
static sockaddr_any src_addr = {{ 0, }, };
static unsigned int src_port = 0;

static const char *module = "default";   // first used in check_progname() 应该指的是调用哪个相应的模块来发包
static const tr_module *ops = NULL;  //  在 do_it 中用到 , ops 应该就是表示 我们在命令中要使用的那个 module

static char *opts[16] = { NULL, };	/*  assume enough   */  // 这个是用来当指定 -O 时，存放对模块里面参数的设置
...
//	static int num_sending = 0;   static int num_sending2 = 0;
	static int num_recieved = 0;
	static int num_sending_probes = 0;
	double * time_interval = 0;

static void print_addr (sockaddr_any *res); 
		static double poisson (double);
// two error handling functions
...
void error (const char *str) {  // error will lead to  exit 
...
// 这个函数里面 按情况对 af,  module 进行了设置
static void check_progname (const char *name) {   // 也就是说traceroute还支持从调用名上选择用什么模块， 通常是用  link file实现（我电脑上就是）
	const char *p;
	int l;

	p = strrchr (name, '/');  //返回最后一次出现 / 的位置
	if (p)  p++;
	else  p = name;
	...
	if (p[l] == '6')  af = AF_INET6; // af is a global varible defined in traceroute.c    , static
	else if (p[l] == '4')  af = AF_INET; // defined as constants in socket.h

	if (!strncmp (p, "tcp", 3))
		module = "tcp";  // module is also a global varible ,   
...
static void init_ip_options (void) {
	...
	if (!num_gateways)  // ?? here num_gateways is 0     85 static int num_gateways = 0;  说明之前早就调用了 add_gateway
		return;
...
static int add_gateway (CLIF_option *optn, char *arg) {
		printf("add_gateway called ,arg is : %s\n",arg);
...
static int set_mod_option (CLIF_option *optn, char *arg) {
		printf("set_mod_option called,  arg is : %s\n" , arg);   // -O sysctl   -O syn   这样要调用两次这个函数
	if (!strcmp (arg, "help")) {     // is  arg == "help"
	    const tr_module *mod = tr_get_module (module);

	    if (mod && mod->options) {
		/*  just to set common keyword flag...  */
		CLIF_parse (1, &arg, 0, 0, CLIF_KEYWORD);
		CLIF_print_options (NULL, mod->options);  //帮助  都是由这个函数打出来的？
	    } else
		fprintf (stderr, "No options for module `%s'\n", module);

	    exit (0);
	}
	//			printf("sizeof (opts) / sizeof (*opts)  is : %d\n",  sizeof (opts) / sizeof(*opts) );  // 这个值实际上是 16
	if (opts_idx >= sizeof (opts) / sizeof (*opts))  { 
	    fprintf (stderr, "Too many module options\n");
	    return -1;
	}

	opts[opts_idx] = strdup (arg);  //strdup (string.h) , 这个函数自己会在堆里 malloc ，然后赋给我们左边的
	if (!opts[opts_idx])  error ("strdup");
	opts_idx++;  //最后一次赋值后会 ++     所以opts_idx 的值是参数的个数
...
// 这里面完成了对  dst_addr的填写，以及dst_name
static int set_host (CLIF_argument *argm, char *arg, int index) {  // set_host 这个函数无论如何都被调且了，即使只  traceroute www.renren.com, arg is   www.renren.com ??
		printf("set_host called, arg is %s\n",arg);
...
static void do_it (void);
		static void wy_print_variable()
		{
		 	print_addr(&dst_addr);  printf("\n");	
			...
			printf("num_probes    : %d\n" , num_probes);	
			printf("dst_port_seq  : %d\n" , dst_port_seq);
			printf("num_gateways  : %d\n" , num_gateways);	
			printf("send_secs     : %.2f\n" , send_secs); //这里要用 %f, 用%d打出来是0 （why?)	
			int i=0;
			for (  ; i<opts_idx ; i++)
				printf("opts[%d]        : %s\n" , i ,  opts[i]);	
		}
int main (int argc, char *argv[]) {
	...
	setlocale (LC_ALL, "");     //make things local
	setlocale (LC_NUMERIC, "C");	/*  avoid commas in msec printed  */

	check_progname (argv[0]);  // here use argv[0] as the parameter


	if (CLIF_parse (argc, argv, option_list, arg_list,                   // ?? this function
				CLIF_MAY_JOIN_ARG | CLIF_HELP_EMPTY) < 0
	)  exit (2);

		//wy_print_varible();
	ops = tr_get_module (module);  // defined in  module.c
	...
	if (src_addr.sa.sa_family && src_addr.sa.sa_family != af)  // 这个 if 没看懂
	...
	if (send_secs >= 10)	/*  it is milliseconds   */
		send_secs /= 1000;  //若send_secs >=10 ,means  milliseconds
	...
	num_probes = max_hops * probes_per_hop;  //  num_probes 的值有了，do_it() 里面最开始就要用
	probes = calloc (num_probes, sizeof (*probes));    //probes 这里分配空间了，  later    probes[n], 注意这里是给（*probes)allocate space , 是 type struct probe
	if (!probes)  error ("calloc");


	if (ops->options && opts_idx > 1) {
	    opts[0] = strdup (module);	    /*  aka argv[0] ...  */  // opts[0] 在这儿就已经赋值了
	    if (CLIF_parse (opts_idx, opts, ops->options, 0, CLIF_KEYWORD) < 0) // 这个怎么相关设置就在 mod-tcp.c 里面看了
		    exit (2);
	}

	if (ops->init (&dst_addr, dst_port_seq, &data_len) < 0)  // 这儿完成了初始化
...
static void print_header (void) {
	...
	/*  Note, without ending new-line!  */   // traceroute to www.taobao.com (121.14.24.251), 30 hops max, 60 byte packets
...
static void print_addr (sockaddr_any *res) {
	//	printf("print_addr called\n");
	const char *str;

	if (!res->sa.sa_family)
	{
			printf("quit print_addr without any print(addr is empty)\n");
	//	return;
	}
	str = addr2str (res);
...
static void print_probe (probe *pb) {
	...
	unsigned int np = idx % probes_per_hop;  // a new line or not 

	if (np == 0)
		printf ("\n%2u ", ttl);


	if (!pb->res.sa.sa_family)   // here ??
		printf (" *");
	else {
...
static void print_end (void) {
	...
	printf ("\n");
		double loss_rate = ((double)num_sending_probes -(double) num_recieved)/(double)num_sending_probes ; 
		printf("%d packets sent , %d packets recieved  , %2f%% packet loss\n" , num_sending_probes, num_recieved, 100*loss_rate);
		//printf("num_sending2 is %d\n", num_sending2);
		printf("the time intervals between each sending pobes :\n");
		int i = 0;	
		for (i=0 ; i<num_sending_probes ; i++)
		{
			printf("%5f ", time_interval[i]);
			if (i % 6 == 0)
				printf("\n");
		}
}
...
static void check_expired (probe *pb) {
	...
	if (!pb->done)	    /*  an ops method still not release it  */
	    return;      // 对  udp_check_expored,  这里是 done 过了
...
static void poll_callback (int fd, int revents) { //可不可以在调用 poll_callback的地方直接使用  ops->recv_probe
...
static void do_it (void) {
	...
	double last_send = 0;  //用来记录当前最后一个包的 probe->send_time
				int i = start ; //used for time_interval
				int ii = 0 ; //used in send_secs assginment each time
			num_sending_probes = end - start + 1;
			time_interval = calloc(num_probes , sizeof(double));			
			if (!time_interval)   error("calloc time_interval");
			printf("the addrss allocated for time_intervall in heap : %p \n ", &time_interval);
				for ( ;i < num_probes ; i++)
					time_interval[i] = poisson (2.0);
	print_header ();  //print_header() 在这儿才调用

	//	static int  num_sending;
	while (start < end) {  // start ,end  表示发包次序数,最初 (end - start+1) 也就是发包的个数
	    int n, num = 0;
	    double max_time = 0;
	    double now_time = get_time ();//get the time now , the time of execution is ignored


	    for (n = start; n < end; n++) {
		probe *pb = &probes[n];  //here it is right ,  probes[n] is not a pointer ( but a struct)
		...
		if (pb->done) {
		    ...
		    if (n == start) {	/*  can print it now   */ //why this condition  这条件保证了是按发包顺序print
			print_probe (pb);
			start++;
		    }

		    if (pb->final)  // final 作什么用? 且这里修改了  end, end关乎 循环条件   pb->final 在module的 _check_reply里面修改了
			end = (n / probes_per_hop + 1) * probes_per_hop;  //final 应该是说 当前包已经探到目的地了,而这里把 end 设为当前包所在组的最后一个，即发完这三个就停 

		    continue;
		}


		if (!pb->send_time) {  //send_secs 是发包间隔时间， wait_secs是发包后的最长等待时间,now_time是现在时间
		    int ttl;

		    if (send_secs && (now_time - last_send) < send_secs) {
			max_time = (last_send + send_secs) - wait_secs;  //这里为什么有 wait_secs,应该是下面timeout的计算
			break;
		    }

		    ttl = n / probes_per_hop + 1;

		    ops->send_probe (pb, ttl); // ttl 是在 set_ttl中，用setsockopt 用来设为 socket 的属性了
					pb->probe_number = n ;			
					send_secs = time_interval[n] ; // printf("the ii in send_secs = time_interval[ii++] : %d   send_secs : %5f\n" , ii , send_secs );
					printf("	probe %d sent	send_secs : %5f\n"  ,n ,send_secs);
					//num_sending++;
		    if (!pb->send_time) {
		    ...
		    last_send = pb->send_time;  //last_send是上次发包时间
		}


		if (pb->send_time > max_time)   // pb->send_time >= max_time
			max_time = pb->send_time;

		num++;  			// for每执行一次，只要不break,continue,      num++
...
void tune_socket (int sk) {
	...
	if (debug) {
	    i = 1;
	    if (setsockopt (sk, SOL_SOCKET, SO_DEBUG, &i, sizeof (i)) < 0)  // enable debug tracing
...
void parse_icmp_res (probe *pb, int type, int code, int info) {
	...
	if (af == AF_INET) {
		...
		switch (code) {
		    ...
		    case ICMP_UNREACH_PORT:
			    /*  dest host is reached   */  // here when dest host is reached
			    str = "<dest reached>";
			    break;
	...
	if (*str) {
	    strncpy (pb->err_str, str, sizeof (pb->err_str));  // 这里设置了 pb->err_str
...
void recv_reply (int sk, int err, check_reply_t check_reply) {  //pb->recv_time   is  got from here
	//		static  int  num_recicved; 
		  
	struct msghdr msg;
	sockaddr_any from;
	struct iovec iov;  //specially defined struct
	int n;
	probe *pb;
	char buf[1280];		/*  min mtu for ipv6 ( >= 576 for ipv4)  */
	char *bufp = buf;
	char control[1024];  // char [] is ok 
	struct cmsghdr *cm;
	struct sock_extended_err *ee = NULL;


	memset (&msg, 0, sizeof (msg));
	msg.msg_name = &from;   //value-result
	msg.msg_namelen = sizeof (from);  //directly assigned
	...
	n = recvmsg (sk, &msg, err ? MSG_ERRQUEUE : 0);  //here , from is filled in msg.msg_name (the destination address for this probe)
	if (n < 0)  return;
	...
	*/
	...
	pb = check_reply (sk, err, &from, bufp, n); //check_reply的作用是根据 sk 返回 pb (至少 udp_check_reply是)
	if (!pb)  return;


	if (!err)
	    memcpy (&pb->res, &from, sizeof (pb->res));  // !! 这里，把 pb->res 填上了, 也就是说没有 POLLERR,才填上 pb->res

		       		printf("probe %d recieved\n", pb->probe_number); 
			        num_recieved++;
	/*  Parse CMSG stuff   */   // ???
...
//				printf("recv_time got through cmsgdata\n");
		...
		if (cm->cmsg_type == IP_TTL)
			pb->recv_ttl = *((int *) CMSG_DATA (cm));  //pb->recv_ttl is for ??
...
//			printf("recv_time got through get_time()\n");
		}
...
void use_recverr (int sk) {  // 这里是设置什么的？
	int val = 1;
...
int do_send (int sk, const void *data, size_t len, const sockaddr_any *addr) {
	...
	if (!addr || raw_can_connect ())   // ? raw_can_connect()  connect 过 就用  send
		res = send (sk, data, len, 0);  //  所用的module 不是用   send 就是 sendto
	else
...
void print_maxmind (sockaddr_any * res)
{
//	printf("\nprint_maxmind called for ip <     ");
//	print_addr (res);   printf(">\n");
	const char * host ;
	host = addr2str (res); 
...
// 这个函数名虽然叫 poisson,但返回的随机数服从的是 指数分布(也就
//是发包间隔时间),使单位时间发包数量服从poisson分布,miu是 间隔期望值 
double  poisson (double miu)
{
		double rand_n = rand();
		double rand_num = rand_n / RAND_MAX ; 
		double result = -miu * log(rand_num);
		printf("time_interval : %5f ,    rand()/RAND_MAX : %5f  ,  rand()  : %5f\n" , result, rand_num, rand_n );
		if (result < 4*miu)
			return result;
		else return 4*miu;
} 
