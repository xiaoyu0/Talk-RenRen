...
static void tcp_recv_probe (int sk, int revents) {   //only tcp_recv_reply has so many codes, others just do the last three lines of codes

	if (sk != icmp_sk) {	/*  a tcp socket   */   //the codes inside this if .. need to be understood later
	    probe *pb;
